...
def main(args):
    ...
    if args.clipseg:
        ...
        clip_seg = ClipSeg(device=device, save_path=logger.lwrf_results_directory)
    else:
        lwrf_model = LwrfInfer(use_cuda=trainer.use_cuda, save_path=logger.lwrf_results_directory)
    ...
    if args.clipseg:
        if is_testing:
            target_object = input("Which color and shape of object do you want to pick? such as a red cube, a yellow cuboid, a blue cylinder?\n")
            # prompt engineering make CLIP to understand what I truly want
            # TODO: 抓取成功的target name 改成 CLIP prompt对应的物体
    ...
    while True:
        ...
        if args.clipseg:
            if is_testing:
                prompts = ["background"]
                prompts.append(target_object)
            else:
                prompts = ["background", "red", "yellow", "green", "orange", "blue", "violet", "indigo"]
            segment_results = clip_seg.segment(color_img, prompts)
        else:
            segment_results = lwrf_model.segment(color_img)
        ...
        if len(mask_heightmaps['names']) == 0 and test_target_seeking:
            ...
            if args.clipseg:
                print('CLIPSeg segments:', mask_heightmaps['names'])
            else:
                print('lwrf segments:', mask_heightmaps['names'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--clipseg', dest='clipseg', action='store_true', default=True)
    parser.add_argument('--lwrf', dest='lwrf', action='store_true', default=False)
