...
class FeatureTunk(nn.Module):
    ...
    def __init__(self, pretrained=True, device="cuda"):
        ...
        # change a efficient model 
        self.model = timm.create_model('efficientnetv2_rw_t',pretrained=True, in_chans=5).to(device)
        
        # create a ConvNeXt model
        # model_name = "convnext_base_384_in22ft1k"
        # # : https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/convnext.py
        # self.model = timm.create_model(model_name, pretrained=True, in_chans=5, num_classes=1024).to(device)
    ...
    def forward(self, color, depth, mask):
        ...
        return self.model.forward_features(torch.cat((self.color_extractor(color), self.depth_extractor(depth), self.mask_extractor(mask)), dim=1))

#
# feature = FeatureTunk()
# a = torch.rand(1,3,640,480)
# b = torch.rand(1,1,640,480)
# c = torch.rand(1,1,640,480)
# feature.forward(a, b, c)
